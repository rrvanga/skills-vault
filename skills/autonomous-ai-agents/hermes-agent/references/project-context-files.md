...
That file at `<REDACTED>/projects/myrepo/.hermes.md` is auto-loaded when Hermes runs in any subdirectory of `<REDACTED>/projects/myrepo`, but not when it runs in `<REDACTED>/other-project`.
