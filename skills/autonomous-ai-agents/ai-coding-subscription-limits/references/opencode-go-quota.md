...
- Current fallback (set 2026-08-15, verified via `hermes config get`): **opencode-go / nemotron-3-ultra-free / `https://opencode.ai/zen/v1` / OPENCODE_GO_API_KEY** — a FREE-tier model on the same key, so a $30/wk dollar-bucket exhaustion now auto-switches to the free tier instead of freezing. (Previously glm-5.2 on zen/go/v1 — same paid bucket, useless for bucket exhaustion.)
