...
- **2026-08-19 addendum**: adaptive monitor flipped fallback to `hy3-free` (nemotron-3-ultra-free still in free=6 but no longer top-healthy in ping ranking) via `hermes config set fallback_providers.model hy3-free`, verified via `hermes config get fallback_providers` (provider opencode-go, base_url zen/v1, key_env OPENCODE_GO_API_KEY). `paid28` added one new model `muse-spark-1.2-contributor` with no official rate (go.mdx still 404) → priced ≈ sibling `muse-spark-1.2` (0.50, 3.00, 0.05, 0.625) marked `# GUESS` in BOTH `token_usage_report.py` and `morning_context.py`; both scripts exit 0 with no unknown-model warnings. Catalog now paid28/zen61/free6.
