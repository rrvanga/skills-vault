...
## Model routing (workers never route themselves)
A worker runs the **assignee profile's own configured model** unless the card carries `model_override`. `_worker_argv` (`hermes_cli/kanban_db_dispatch.py`) appends `-m <model> [--provider <name>]` **only** when that override exists, and `hermes config get kanban` has no model/provider key — so there is no board-wide default to set, and a board with no overrides is routing nothing at all.
- **Per-card at creation:** `hermes kanban create --model <m> --provider <p> ...`; the `kanban_create` tool takes the same `model`/`provider` args (`provider` requires `model`).
- **Per-card retroactively:** `hermes kanban set-model <task_id> <model> [--provider <p>]` — applies on the NEXT dispatch, so it is also the recovery move when a running model hits a rate limit. Clear with `set-model <id> none`.
- Reasoning effort rides the same path (`tasks.reasoning_effort`); the dashboard card dropdown writes the `provider::model` override pair.
- **Diagnose in one query:** `sqlite3 ~/.hermes/kanban.db "SELECT count(*), sum(model_override<>'') FROM tasks;"` — if the second number is 0, nothing has ever been routed.
- **Prove what would spawn without spending tokens:** `hermes kanban dispatch --dry-run` prints the worker command it would launch (read-only, no spawn).
- **Don't confuse sources of truth:** the desktop app's model picker is app-local (`hermes.desktop.model-presets` in `~/.config/Hermes/Local Storage/leveldb`) and propagates nowhere — profile configs, cron (`cron.model`), and the board each carry their own. Pick the model per task class with the `llm-model-selection` skill; current GO rates live in `~/.hermes/scripts/token_usage_report.py` (`GO_RATES`), which is also the quota meter.

## Class → model rule (the convention this box runs on)
Mechanism chosen 2026-09-19: **creation-time flags, no daemon, no sweep, profiles untouched.** Every card is created with an explicit pin; nothing infers it later. Figures below are per-card $ at ~50 calls (7.5K in / 2.3K out / 34K cached) against Go's **per-model** limits — a $/card figure is meaningless without the cap it burns.

| task class | pin | per-card | own cap | cards before cap | req/5h cap |
|---|---|---|---|---|---|
| bulk mechanical: file sweeps, renames, index/OCR passes, log triage, harvest | `deepseek-v4-flash` | ~$0.16 | $30/mo | ~191 | 13,000 |
| prose/notes/docs, ordinary research reads, summaries | `glm-5.3-flash` | ~$0.17 | $60/mo | ~359 | 6,320 |
| long-context analysis, multi-source synthesis | `deepseek-v4-pro` | ~$0.62 | **$15/mo** | **~24** | 1,050 |
| code implementation: repos, PRs, tests, refactors | `kimi-k2.7-code` | ~$1.15 | $60/mo | ~52 | 1,350 |
| hard reasoning: gnarly debugging, architecture, gnarly design calls | `kimi-k3` | ~$3.38 | **$15/mo** | **~4** | **110 (≈2 cards / 5h)** |

Usage rules:
1. **Always pin at creation** — `hermes kanban create --model <m> --provider opencode-go ...` (same `model`/`provider` args on the `kanban_create` tool). The assignee profile's seeded `deepseek-v4-flash` is a **default that was never a decision**; leaving it is only correct for the top row of the table.
2. **Fan-out children do not inherit the parent's pin** (`_clean_children` carries only title/body/assignee/parents) — pin each child, or the decomposition silently drops to flash.
3. **Escalate on evidence, not on vibe.** `kimi-k3` and `deepseek-v4-pro` are rationed lanes (~4 and ~24 cards/5h-ish lifetime): reach for them for one hard step, then pin the follow-up child back down.
4. **A rate limit is a routing event**, not a retry: `hermes kanban set-model <id> <next-model> --provider opencode-go` takes effect on the NEXT dispatch.
5. **Never re-pin a card mid-run** — the override is read when the worker is spawned; changing it affects the next dispatch, so verify with `dispatch --dry-run`.
6. **Check the true ceiling before filling a lane:** `python3 ~/.hermes/scripts/token_usage_report.py` — read the **Per-model 30d** line, not the pool line. As of 2026-09-19 the whole board runs `deepseek-v4-flash`, which sits at **53.5% of its own $30 cap** while the pool line flatters it to 27.3%.
...
9. **Cards can be hard-DELETED from the DB, not just archived.** A task can vanish mid-run so that `show` returns "no such task" AND `task_events`/`task_runs`/`task_links` rows for it are all gone (verified 2026-09-19: the entire "mint to tello" chain — root t_40d475f6, gate t_31cc2bc0, worker card t_f621d5de — was removed between a `show` reporting `running` and the worker's `complete` call minutes later; `complete` then failed with "unknown id or terminal state"). The board still lists done siblings; only the chain's own rows are purged. **If your card disappears mid-run: do NOT retry or re-create it; save your report to a durable folder (e.g. the topic's artifact dir under ~/Documents/...), not just /tmp, and report the deletion as the outcome.**
10. **Cleanup = `archive` — there is no `delete` verb; retiring a topic needs a full sweep, not one card.** Archiving keeps the card + its events visible under `hermes kanban list --archived` (add `--json` for machine-filterable rows), so it is the safe "delete". Two traps: (a) an earlier archived card does NOT stop a NEW card on the same topic being created later — before declaring a topic retired, sweep BOTH the active board and `list --archived --json` with topic keywords; (b) also check cron jobs and leftover watch scripts — an archived card's comments may reference a script that is still scheduled (verified 2026-08-31: GPU price cards — one archived 8/24, a second card re-created 8/29 still parked; both now archived, and the sweep confirmed no cron/script still ran the watch).
11. **A bot's model lives in its profile config, and that config is a seeded COPY — it never follows later changes.** `_seed_model_config()` (`hermes_cli/profiles.py`) *copies* the model block into `~/.hermes/profiles/<name>/config.yaml` at profile-creation time ("Copy (not link)"), so changing the global/default model afterwards reaches neither existing bot profiles nor the board. When a card "uses the wrong model", read `~/.hermes/profiles/<assignee>/config.yaml` — that file, not the root config, is the worker's truth. Scope the hunt: never recursive-grep `~/.hermes` for model strings (the multi-MB `models_dev_cache.json` / `provider_models_cache.json` flood or blow up the output); read the profile config and query `kanban.db` directly.
12. **There is no task-created hook, so auto-routing cannot be event-driven.** The kanban lifecycle hook set is exactly `on_kanban_dispatch_tick`, `on_kanban_task_updated`, `on_kanban_worker_spawned`, `on_kanban_worker_exited`, `on_kanban_worker_stale_claim`, and `_fire_dispatch_tick_hook` runs strictly AFTER the tick's claim/spawn, outside the lock — so no hook or plugin can set the model of the task being claimed. Build automatic routing as a **forward sweep** (a bounded cron that scans `ready` cards with an empty override and calls `set-model`, keyed on assignee + title/body keywords) or as creation-time flags; don't spend an afternoon designing a hook that cannot exist. Related gap: fan-out children are created with only `title/body/assignee/parents` (`_clean_children`, `kanban_decompose.py`; same for `kanban_swarm.py`), so a parent's `model_override` is NOT inherited by its children — set the pin on children explicitly or let the sweep backfill them.
