...
- **A generative agent job's PROMPT can re-create work the user asked to stop**: a standing directive baked into an agent job's prompt (e.g. an "AI-desktop build shopping: re-check prices" candidate in an autonomy window) re-fires that task EVERY tick regardless of the kanban board or any script — archiving cards or pausing a no_agent script job does NOT stop it. When the user says "stop doing X" and a recurring agent job does X, scrub the directive from that job's PROMPT itself (candidate list, step list, examples), not just the artifacts around it. Then sweep EVERY layer for the topic keyword and assert each clean: kanban board, ALL job prompts, `~/.hermes/scripts/` — a hit in any one of them means the work can resurface.
