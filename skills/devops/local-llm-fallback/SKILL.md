...
- FLOCK FD INHERITANCE (2026-09-05, caused 15h of silent watchdog death): the global `flock -n 9` mutex fd is inherited by the daemonized llama-server child (`setsid nohup ... &`). Once the child holds fd 9, the flock never releases when the script exits — every later tick dies at `flock -n 9` → silent `exit 0` → status "ok" → stop/start logic NEVER runs. The child must close it: append `9>&-` to the daemonized command's redirections. Diagnose with `fuser ~/.hermes/.cache/local_backup_watchdog.lock` (shows the server PID holding it) or `bash -x` (trace exits right after `flock -n 9`). Also added: cloud-healthy branch now reconciles `STATE_NOW=down` when the server is already down (prevents stale "up" state file after manual stops). Verify with `bash -x` that the trace reaches `cloud_healthy` instead of dying at the flock.
