...
message.send.backend.login = "<REDACTED>"
...
```bash
himalaya envelope list from <REDACTED> subject meeting
```
...
himalaya template forward 42 | sed 's/^To:.*/To: <REDACTED>/' | himalaya template send
```
...
```bash
himalaya message write -H "To:<REDACTED>" -H "Subject:Test" "Message body here"
```
